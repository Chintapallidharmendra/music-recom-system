...
from monitoring.metrics import push_drift_metrics, track_parquet_load
...
def run(bootstrap_servers: str, reference_size: int, window_size: int, check_every: int) -> None:
    ...
    logger.info(
        "listening on '%s' -- filling reference window (%d plays) before checks start",
        SYNTHETIC_PLAYS_TOPIC, reference_size,
    )
    print(
    ...
    for message in consumer:
        ...
        if len(reference_rows) < reference_size:
            ...
            if len(reference_rows) == reference_size:
                logger.info("reference window filled (%d plays) -- checks starting", reference_size)
        ...
        push_drift_metrics(summary, job="drift_stream_monitor")
...
def main():
    ...
    setup_logging("drift_stream_monitor")
