...
def compute_drift(reference: pd.DataFrame, current: pd.DataFrame) -> tuple[dict, Report]:
    if len(current) < MIN_CURRENT_EVENTS:
        logger.info(
            "skipping drift check: only %d/%d current events available",
            len(current), MIN_CURRENT_EVENTS,
        )
        summary = {
            "dataset_drift": False,
    ...
    log_level = logging.WARNING if summary["dataset_drift"] else logging.INFO
    logger.log(
        log_level,
        "drift check complete dataset_drift=%s drifted_columns=%d/%d current_events=%d",
        summary["dataset_drift"], summary["number_of_drifted_columns"],
        len(DRIFT_COLUMNS), summary["current_events"],
    )
    return summary, report


def main():
    setup_logging("drift_report")
    ...
    logger.info(
        "reference window: %d events, current window: %d events", len(reference), len(current)
    )
    ...
    if not args.no_push:
        from monitoring.metrics import push_drift_metrics
        push_drift_metrics(summary, job="drift_report")

    if summary["monitoring_ready"]:
        report.save_html(args.out)
        logger.info("saved HTML drift report to %s", args.out)
        print(f"saved HTML report to {args.out}")
    else:
        logger.info("waiting for %d live events before drift monitoring", MIN_CURRENT_EVENTS)
