...
def _check_drift(**context) -> str:
    ...
    # Prometheus (push, since this task exits before it could ever be scraped)
    try:
        from monitoring.metrics import push_drift_metrics
        push_drift_metrics(summary, job="airflow_retrain_policy")
    except Exception as exc:  # noqa: BLE001 -- monitoring must never break the DAG
        print(f"could not push drift metrics to Prometheus Pushgateway: {exc}")
